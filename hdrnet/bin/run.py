...
def get_input_list(path): # 可以使用文件夹/filelist.txt/单张图片的路径作为输入
...
def main(args):
  ...
  with tf.Session(config=config) as sess:
    ...
    tf.train.import_meta_graph(metapath) # 加载checkpoint模型

    model_params = utils.get_model_params(sess) # 获取
  ...
  with tf.Session(config=config) as sess:
    ...
    for idx, input_path in enumerate(inputs):
      ...
      out_ =  sess.run(output, feed_dict=feed_dict) # 用feed_dict中的数据作为输入，来进行graph的运算以计算出output
